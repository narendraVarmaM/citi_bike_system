...
def plot_prediction(features: pd.DataFrame, prediction: pd.DataFrame):
    """
    Plots time series data and a prediction point, including location name in the title.
    """
    ...
    time_series_dates = pd.date_range(
        start=pickup_hour - timedelta(hours=4*len(time_series_columns)),
        end=pickup_hour,
        freq="4h",
    )
    ...
    # Create plot title with location name and other metadata
    title = f"Location: {prediction['Zone'].iloc[0]}, Pickup Hour: {pickup_hour}"
